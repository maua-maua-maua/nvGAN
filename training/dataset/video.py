...
class DecordVideoDataset(Dataset):
    def __init__(
        ...
        **super_kwargs,  # Additional arguments for the Dataset base class.
        ...
        self.sampling_kwargs = dict(type="random", dists=[1, 2, 4, 8, 16, 32], num_frames_per_sample=2)
        self._path = path
        self._zipfile = None
        ...
        if os.path.isdir(self._path):
            ...
            self._files = list(
                {
                    file
                    for file in listdir_full_paths(self._path)
                    if os.path.splitext(file)[1] in {".mp4", ".webm", ".mkv", ".avi", ".gif"}
                }
            )
        elif os.path.splitext(self._path)[1] == ".zip":
            self._type = "zip"
            self._files = list(
                {
                    file
                    for file in self._get_zipfile().namelist()
                    if os.path.splitext(file)[1] in {".mp4", ".webm", ".mkv", ".avi", ".gif"}
                }
            )
        else:
        ...
        raw_shape = [len(self._files)] + list(self._load_raw_frames(0)[0][0].shape)
    ...
    def _load_raw_labels(self):
        ...
        labels = dict(labels)
        labels = [labels[filename] for filename in self._files]
    ...
    def __getitem__(self, idx: int) -> Dict:
        frames, times, video_len = self._load_raw_frames(self._raw_idx[idx], frames_idx=None)
    ...
    def _load_raw_frames(self, raw_idx: int, frames_idx: List[int] = None) -> Tuple[np.ndarray, np.ndarray]:
        vr = VideoReader(self._open_file(self._files[raw_idx]), width=self._resolution, height=self._resolution)
        total_len = len(vr)
        ...
        images = vr.get_batch(frames_idx).asnumpy().transpose((0, 3, 1, 2))

        return images, frames_idx - offset, min(self.max_num_frames, total_len)
...
class VideoFramesFolderDataset(Dataset):
    def __init__(
        ...
        **super_kwargs,  # Additional arguments for the Dataset base class.
        ...
        if os.path.isdir(self._path):
            ...
            self._all_objects = {
                os.path.relpath(o, start=os.path.dirname(self._path)) for o in {self._path}.union(self._all_objects)
            }
        elif os.path.splitext(self._path)[1] == ".zip":
            self._type = "zip"
        ...
        root_path_depth = 0
        curr_d = os.path.dirname(objects[1])

        for o in objects[1:]:
            curr_obj_depth = len(os.path.normpath(o).split(os.path.sep))

            if os.path.splitext(o)[1] in PIL.Image.EXTENSION:
            ...
            elif os.path.splitext(o)[1] == "json":
    ...
    def __getitem__(self, idx: int) -> Dict:
        if self.load_n_consecutive:
            ...
            assert (
                num_frames_available - self.load_n_consecutive * self.subsample_factor >= 0
            ...
            if self.load_n_consecutive_random_offset:
                ...
                random_offset = 0
            frames_idx = (
                np.arange(0, self.load_n_consecutive * self.subsample_factor, self.subsample_factor) + random_offset
            )
        else:
            frames_idx = None
    ...
    def _load_raw_frames(self, raw_idx: int, frames_idx: List[int] = None) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if frames_idx is None:
            if total_len > self.max_num_frames:
                offset = random.randint(0, total_len - self.max_num_frames)
            frames_idx = (
                sample_frames(total_video_len=min(total_len, self.max_num_frames), **self.sampling_kwargs) + offset
            )
        else:
...
def random_frame_sampling(
    ...
    dists: Union[List, Tuple],
    max_time_diff: int = float("inf"),
    max_dist: int = float("inf"),
    use_fractional_t: bool = False,
    **kwargs,
) -> np.ndarray:
...
if __name__ == "__main__":
    ...
    dl = torch.utils.data.DataLoader(VideoFramesFolderDataset("/home/hans/trainsets/v22small.zip", resolution=256), batch_size=16)
    i=0
    t=time()
    for batch in dl:
        if i == 10:
        ...
        i+=1
    print("VideoFramesFolderDataset:", (time()-t)/10, "sec per batch")

    dl = torch.utils.data.DataLoader(DecordVideoDataset("/home/hans/datasets/2022/videos/", resolution=256), batch_size=16)
    i=0
    t=time()
    for batch in dl:
        if i == 10:
        ...
        batch["image"] = batch["image"].shape
        i+=1
    print("DecordVideoDataset:", (time()-t)/10, "sec per batch")